...
/**
 * @file
 * Varbase Component test coverage metrics.
 *
 * Parses PHPUnit coverage reports and outputs metrics for GitLab.
 */
...
foreach (explode("\n", $output) as $line) {
  ...
  if (!empty($m)) {
    ...
    file_put_contents($metricsFilePath, "varbase.coverage.{$metric} {$metricValue}\n", FILE_APPEND);
  }
}
