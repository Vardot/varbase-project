Feature: Website Base Requirements - Website Languages - All content translatable to all languages
  ...
  # TODO(10.1.x CI): on Drupal 11.4 the page redirects to a 404 immediately
  # after saving the Arabic (RTL) translation, so the Gin top-bar page-actions
  # button is gone by the next step. Needs a browser to trace the post-save
  # redirect (language prefix / path alias). Un-@wip once fixed.
