#!/usr/bin/env bash
################################################################################
# Make varbase-e2e's getLocatorText() <select>-aware for the CI run.
#
# Called by the .fragments node_playwright before_script (after `yarn install`,
# so node_modules/varbase-e2e is present). Idempotent; a genuine mismatch still
...
# varbase-e2e's getLocatorText() (used by the "I should see X in the Y
...
WS="$CI_PROJECT_DIR/node_modules/@vardot/varbase-e2e/tests/step-definitions/varbase-e2e.js"
